...
function App() {
  ...
  const [employees, setEmployees] = useState<IEmployee[]>(DUMMY_EMPLOYEES);
  const [teams, setTeams] = useState<ITime[]>(DUMMY_TEAMS);
  ...
  const onChangeTeamColor = (id:string, color:string) => {
    setTeams(
      teams.map((team) => {
        if (team.id === id) {
          team.color = color;
